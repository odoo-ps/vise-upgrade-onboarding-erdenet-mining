...
#### Prepare the upgraded database dump
- merge filestore: cp -r ~/.local/share/Odoo/filestore/{old_database_name}/ ~/.local/share/Odoo/filestore/{new_database_name}
- restore upgraded dump: unzip {zip} && createdb {new_database_name} && psql {database_name} < dump.sql
- overwrite new filestores from dump: cp -r filestore/* ~/.local/share/Odoo/filestore/{new_database_name}/
- run database

#### Bugs found only after upgrading
Unlike the section 2 bugs (found while still exploring the 16.0 database), these only surface once the database has actually gone through the upgrade to 19.0 -> the underlying cause is data/view corruption that the module's own install/upgrade code has no way to detect on its own.

- Website (multi-website) -> load any front-end page on the second website -> `AttributeError: 'website' object has no attribute 'sale_get_order'`:
-> `website_sale.header_cart_link` was COWed for that website through a front-end edit (per-website `ir.ui.view`, `key='website_sale.header_cart_link'`), then its arch was manually reverted to the 15.0 version, which still calls `website.sale_get_order()` in QWeb
-> `sale_get_order` was removed from `website_sale`'s `website` model override in 19.0 (cart resolution moved to `request.cart`/session caching); since it's a per-website COW, no generic `upgrade/migrations` script tracks or repairs it -> only customer-specific `reset_cowed_views()` patches exist for this key, in `upgrade-specific`
-> Fix: `util.reset_cowed_views(cr, "website_sale.header_cart_link")` added to `estate/migrations/19.0.1.0.0/end-migrate.py` (end stage, so the module's stock arch is already loaded before it gets copied over the COWed row)

- Accounting -> browse Invoicing > Accounting after upgrading -> "Analytic Items" no longer sits where the 19.0 menu structure expects it:
-> `account.menu_action_analytic_lines_tree` was `noupdate=True` -> its `parent_id` was `account.menu_finance_entries_management`, which 19.0's `account_menuitem.xml` removes entirely, moving this menu under `account_transactions_menu` instead -> the noupdate-protected record never picked up that move
-> Fix: `estate/migrations/19.0.1.0.0/end-migrate.py` -> directly sets `parent_id` to `account.account_transactions_menu` and clears `noupdate` on the record

- Discuss App -> Migration report -> Section "Automation Rules" -> Some records have been archived
